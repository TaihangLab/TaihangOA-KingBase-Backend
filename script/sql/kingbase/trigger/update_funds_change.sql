-- 人大金仓版本: update_funds_change 触发器
-- 在 project_funds 表 UPDATE 后触发

-- 先删除已存在的触发器和函数
DROP TRIGGER IF EXISTS update_funds_change ON project_funds;
DROP FUNCTION IF EXISTS update_funds_change_func();

-- 创建触发器函数
CREATE OR REPLACE FUNCTION update_funds_change_func()
RETURNS TRIGGER AS $$
BEGIN
...
-- 创建触发器
CREATE TRIGGER update_funds_change
    AFTER UPDATE ON project_funds
    FOR EACH ROW
    EXECUTE FUNCTION update_funds_change_func(); 